        ...
        <>
            <div className="fixed inset-0 z-[100] flex items-start lg:items-center justify-center pt-0 lg:pt-4">
                ...
                <div className="relative w-full max-w-[500px] bg-white dark:bg-black rounded-none lg:rounded-2xl shadow-2xl overflow-hidden flex flex-col h-full lg:h-auto max-h-[100vh] lg:max-h-[600px] lg:min-h-[280px]">
                    ...
                    {/* Email Verification Warning – clicking opens the full modal */}
                    {currentUser && !currentUser.emailConfirmed && (
                        <button
                            onClick={() => setIsVerifyEmailOpen(true)}
                            className="w-full flex items-center gap-3 bg-[#006aff]/10 hover:bg-[#006aff]/15 transition-colors px-4 py-3 border-b border-[#006aff]/20 text-left"
                        >
                        ...
                        {searchQuery && (
                            <div className="flex flex-col">
                                {loading ? (
                                    ...
                                    results
                                        .filter((user) => {
                                            // In group mode, show all users
                                            if (isGroupMode) return true;
                                            
                                            // In 1-on-1 mode, only show users who follow you
                                            const userKey = user.did || user.id;
                                            const followsMe = userFollowerStatus.get(userKey);
