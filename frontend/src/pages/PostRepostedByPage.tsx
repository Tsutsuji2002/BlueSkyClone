...
import { fetchPostRepostedBy, followUserAsync, unfollowUserAsync } from '../redux/slices/userSlice';
...
const PostRepostedByPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        dispatch(fetchPostRepostedBy({ postUri, limit: INITIAL_LIMIT }))
            .unwrap()
        ...
        const observer = new IntersectionObserver(
            entries => {
                if (entries[0].isIntersecting) {
                    setLoading(true);
                    dispatch(fetchPostRepostedBy({ postUri, cursor, limit: NEXT_LIMIT }))
                        .unwrap()
                        .then(({ users: fetched, cursor: nextCursor }) => {
                            setUsers(prev => {
...
export default PostRepostedByPage;
