...
public class ChatService : IChatService
{
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                { 
                    ...
                    if (isRequest.HasValue)
                    {
                        ...
                        {
                            // For regular conversations: show accepted conversations
                            proxyConvos = proxyConvos.Where(c => c.IsAccepted).ToList();
                        }
                    }
    ...
    public async Task<MessageDto> SendMessageAsync(Guid userId, string conversationId, string? content, string? imageUrl = null, string? replyToId = null, LinkPreviewDto? linkPreviewDto = null, string? replyToRev = null)
    {
        ...
        if (!string.IsNullOrEmpty(token) && !IsGuid(conversationId))
        {
            return await _chatProxy.SendMessageAsync(token, conversationId, content ?? "", replyToId, replyToRev, imageUrl, linkPreviewDto);
        }
    ...
    {
        var user = await _unitOfWork.Users.GetByIdAsync(userId);
        if (user == null || string.IsNullOrEmpty(user.Did)) return (false, "User not found");
